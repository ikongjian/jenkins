...
<body>
   ...
   <!--选择-->
    ...
    <!--头部-->
	<div class="soso">
	  <form id="queryfrom" method="post" action="/qualityCheck/xjlist">
    ...
    <!--项目-->
	<div class="j_tab">
    	<p class="j_tabp" data-id="0">巡检项目</p>
    ...
    <div class="j_nav">
        <ul>
            <li><a  href="#" <c:if test="${templateType == 'all' || templateType == null}">class="j_navHover"</c:if> onclick="searchType('all')">全部</a></li>
            <li><a href="#" <c:if test="${templateType == 'last'}">class="j_navHover"</c:if> onclick="searchType('last')">最近搜索项目</a></li>
            <c:forEach items="${templateTypeList}" var="temp">
	            <li><a href="#" <c:if test="${templateType == temp.code}">class="j_navHover"</c:if> onclick="searchType('${temp.code}')">${temp.text}</a></li>
            </c:forEach>
        </ul>
    </div>
    ...
    <div class="j_content">
       <c:forEach var="item" items="${list}">
          ...
          <tr>
            <td align="center">开工日期</td>
            <td><fmt:formatDate value="${item.workingDate}" pattern="yyyy-MM-dd"/></td>
          </tr>
          <tr>
             <td colspan="2" scope="row">
             <div style="display:block; text-align:center; margin:auto;">
             <a href="#" name="checkview"  data-checkid="${item.checkId}" data-orderno="${item.orderNo }" data-type="10">到达现场</a>
             <a href="#" name="checkdeal"  data-checkid="${item.checkId}" data-orderno="${item.orderNo }" data-type="10" data-sign="${item.signStatus}"
                <c:if test="${item.signStatus != 1}">style="color: gray;"</c:if>>
             	检查项目
...
function searchType(type){
	$("input[name=templateType]").val(type);
	$("#queryfrom").submit();
}
function search(){
