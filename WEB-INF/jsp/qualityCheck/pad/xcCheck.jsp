...
<head>
    ...
    <style type="text/css">
        ...
        .view_pic {
            width: 8rem; float:left; display: block; border: 0.1rem solid #ed6c00; color: #ed6c00; font-size: 1.4rem; height: 2.5rem;
            line-height: 2.5rem; border-radius: 0.4rem; margin-left: 2rem; text-align:center; cursor:pointer;
        }
    </style>
    ...
    <form action="/fujian/check/xcUpdate" method="post" id="xcUpdateForm">
        ...
        <div class="projectForm projectTableFrom">
            <div class="formGroup" style="overflow:hidden;">
                <label>期望检查日期</label>
                <p class="iconForward" style="width:50%; float:left;">
                    <input class="taskInputdate input_date" type="date" name="checkDate">
                </p>
                <c:if test="${!empty imgs}">
                    <a class="lookPic fancybox-buttons view_pic" data-fancybox-group="button" href="${domain}${imgs[0].imgUrl}">查看照片</a>
                    <c:forEach items="${imgs}" var="img" varStatus="status">
                        <c:if test="${status.index>0}">
                            <a class="lookPic fancybox-buttons view_pic" data-fancybox-group="button" href="${domain}${img.imgUrl}" style="display:none;"></a>
                        </c:if>
                    </c:forEach>
                </c:if>
            </div>
