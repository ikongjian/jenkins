    ...
    <script type="text/javascript">
        ...
        $(function() {
            $("#saveForm").submit(function() {
                ...
                var reasonSize=$("input:checkbox:checked").length;
                var reasonComment=$("#recallComment").val();
                var reasonCommentLen=reasonComment.length;
                if(imgSize==null||imgSize<1||isNaN(imgSize)) {
                    layer.msg("请上传至少一张图片！",{icon:6,time:3000});
                    return false;
                } else if(reasonSize==0) {
                    layer.msg("请选择返补原因！",{icon:6,time:3000});
                    return false;
                } else if(reasonComment==null||reasonComment=="") {
                    layer.msg("请填写返补说明！",{icon:6,time:3000});
                    return false;
                } else if(reasonCommentLen>300) {
                    layer.msg("返补说明最多输入300个字！",{icon:6,time:3000});
                    return false;
                } else {
...
<body>
<form action="/wx/taskRecall" method="post" id="saveForm" enctype="multipart/form-data">
    <input type="hidden" name="originPkgId" value="${originPkgId}"/>
    <div class="complete">
        <div class="grab_head">
        ...
        <div class="broadcast">
            <div style="margin-left:20px;margin-top:20px;font-size:1rem;">请上传返补图片：</div>
        ...
        <div class="broadcast">
            <div style="margin-left:20px;margin-top:20px;font-size:1rem;">返补原因：</div>
            <div style="padding:20px; line-height:2rem; overflow:hidden;">
                <label style="float:left; width:50%; font-size:1rem;"><input type="checkbox" name="recallReason" value="现场不具备条件撤场">现场不具备条件撤场</label>
                <label style="float:left; width:50%; font-size:1rem;"><input type="checkbox" name="recallReason" value="材料不足">材料不足</label>
                <label style="float:left; width:50%; font-size:1rem;"><input type="checkbox" name="recallReason" value="质量运输问题">质量运输问题</label>
                <label style="float:left; width:50%; font-size:1rem;"><input type="checkbox" name="recallReason" value="其它">其它</label>
            </div>
            <div style="padding:0 20px 30px;">
                <textarea style="padding:5px 10px; width:100%; height: 90px;" id="recallComment" name="recallComment" placeholder="请输入返补说明，最多输入300字"></textarea>
            </div>
            <input class="button submit_button" type="submit" value="申请返补">
        </div>
    </div>
</form>
</body>
